...
const staffSchema = z
  .object({
    ...
    contactNo: z
      ...
      .regex(
        /^09\d{9}$/,
        "Must be a valid 11-digit mobile number starting with 09",
      ),
...
export default function StaffRegistrationPage() {
  ...
  useEffect(() => {
    if (firstName) {
      setValue("username", firstName.toLowerCase().replace(/\s+/g, ""), {
        shouldValidate: true,
      });
    }
  }, [firstName, setValue]);
  ...
  useEffect(() => {
    if (firstName && contactNo) {
      const generatedPassword = `${firstName}.${contactNo}`;
      setValue("password", generatedPassword, { shouldValidate: true });
      setValue("confirmPassword", generatedPassword, { shouldValidate: true });
    }
  }, [firstName, contactNo, setValue]);

...
export async function GET(req: NextRequest) {
  try {
    ...
    const meterReader = await queryOne<
      { Assigned_Area_ID: string } & RowDataPacket
...
export async function POST(req: NextRequest) {
  try {
    ...
    const defaultPassword = process.env.DEFAULT_CONSUMER_PASSWORD;
    if (!defaultPassword) {
      throw new Error("DEFAULT_CONSUMER_PASSWORD environment variable is not set");
    }
    ...
    const highestConsumer = await queryOne<
      { Consumer_ID: string } & RowDataPacket
    >(

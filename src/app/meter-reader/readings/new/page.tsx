...
export default function RecordMeterReadingPage() {
  ...
  const { data: consumers } = useQuery<Consumer[]>({
    ...
    enabled: hasAccess && search.length > 1,
  })
      ...
      >
            ...
            <div className="flex items-center justify-between bg-primary-50 border border-primary-200 rounded-lg px-4 py-3">
              <div>
                ...
                <p className="text-xs text-primary-600 font-mono">
                  {selectedConsumer.consumerId} — Meter: {selectedConsumer.meterSerialNo}
                </p>
              </div>
              <button
                type="button"
        ...
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3">
          ...
          <p className="text-sm text-amber-700">
            Submitting this form will automatically generate the bill and send
            an SMS notification to the consumer's registered mobile number.
          </p>
        </div>

...
export default function CashierCollectionProgress({ progress, isLoading }: Props) {
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-6 flex flex-col gap-8">
      ...
      {/* Not Yet Paid List */}
      <div>
        <h3 className="text-sm font-semibold text-muted-foreground mb-4 tracking-wider uppercase">Consumers who haven't paid yet</h3>
        ...
        {isComplete ? (
          ...
          <div className="flex flex-col gap-3">
            {progress.notYetPaidList.map((consumer) => (
              <div key={consumer.consumerId} className="flex justify-between items-center py-3 border-b border-border/50 last:border-0">
                <div>
                ...
                <div className="text-right">
                  <p className="text-base font-bold text-[#8c6b23] whitespace-nowrap">
                    ₱{Number(consumer.balance).toLocaleString('en-PH', {
                      minimumFractionDigits: 2,
                    })}
                  </p>
                  <span className="bg-[#fdf0e6] dark:bg-[#4a1f0f] text-[#b83d1c] dark:text-red-300 text-xs font-medium px-3 py-1.5 rounded-full whitespace-nowrap inline-block mt-1">
                    Unpaid
                  </span>
                </div>
              </div>
